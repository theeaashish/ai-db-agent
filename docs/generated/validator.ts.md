...
Validates if a given month and year combination represents a future or current expiration date.

The validation ensures the date is not in the past relative to the current month and year.
...
| `year` | `number` | The full expiration year (e.g., 2025). |

**Returns:**

`true` if the expiration date is valid (current or future), `false` otherwise.
